...
public class WishlistUiTests extends UiBaseTest {
    ...
    public void tc011_wishlistNotLoggedIn() {
        clearSession();
        // Navigate to /wishlist via the Angular router without a page reload
        navigateToRoute("/wishlist");
        // Wait for the router to settle on either /login (guard redirected) or /wishlist
        wait.until(ExpectedConditions.or(
                ExpectedConditions.urlContains("/login"),
                ExpectedConditions.urlContains("/wishlist")
        ));
        boolean redirectedToLogin = driver.getCurrentUrl().contains("/login");
        // If the guard did NOT redirect, the wishlist heading must NOT be rendered
    ...
    public void tc012_moveWishlistToCart() {
        ...
        clearSession();
        loginViaUi(buyerEmail, "Test@1234");
        // Hard wait — if .account-btn is not visible, login silently failed; fail the test now.
     ...
     * Bug: re-clicking does NOT decrease the wishlist count — the item is not removed; the
     * application is supposed to toggle (add/remove) but silently fails on the second click.
     ...
     * Bug: the button always shows class='btn-secondary btn-pill' and text='❤️ Wishlist' —
     * it never updates to reflect the added state (e.g. filled heart, btn-primary, 'Remove' text).
     * This test is expected to FAIL until the application bug is fixed.
     */
    ...
    public void tc021_wishlistButtonChangesVisualState() {
        ...
        var btnsAfter = driver.findElements(By.xpath(
                "//button[contains(.,'Wishlist') or contains(.,'Remove') or contains(.,'Added') or contains(.,'💔')]"));
